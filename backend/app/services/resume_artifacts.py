...
from app.services.resume_artifact_ai import (
    ...
    ResumeArtifactOutputValidationError,
    ResumeArtifactGenerationUnavailableError,
...
class ResumeArtifactSeedMissingError(ResumeArtifactError):
    pass


class ResumeArtifactRoleNotFoundError(ResumeArtifactError):
    pass


class ResumeArtifactEvaluationNotFoundError(ResumeArtifactError):
    pass
...
class ResumeArtifactUnavailableError(ResumeArtifactError):
    pass


class ResumeArtifactProviderError(ResumeArtifactError):
    pass


class ResumeArtifactValidationError(ResumeArtifactError):
    pass


class ResumeArtifactTruthfulnessError(ResumeArtifactValidationError):
    pass
...
class ResumeArtifactService:
    def __init__(
        ...
        generator: OpenAIResumeArtifactGenerator | None = None,
    ) -> None:
        self.db = db
    ...
    def generate_for_role(
        ...
        *,
        ...
        if source_version is None:
            raise ResumeArtifactSourceNotFoundError("Resume source version not found")

        artifact_id = uuid.uuid4()
        self._log_activity(
            user_id=user.id,
            entity_id=artifact_id,
            action="resume_artifact.started",
            details={
                "role_id": str(role.id),
        ...
        try:
            ...
            persisted = GeneratedArtifact(
                ...
                artifact_metadata={
                    ...
                    "target_evaluation_id": str(evaluation.id),
                    "workspace_id": str(payload.workspace_id),
                },
            )
            ...
            self._log_activity(
                user_id=user.id,
                entity_id=artifact_id,
                action="resume_artifact.completed",
                details={
                    "role_id": str(role.id),
                    "evaluation_id": str(evaluation.id),
                    "workspace_id": str(payload.workspace_id),
            ...
            self.db.commit()
            return artifact
        except ResumeArtifactGenerationUnavailableError as exc:
    ...
    def _build_artifact_contract(
        ...
        *,
        ...
        self._enforce_truthfulness(
            output=output,
            source_text=source_version.raw_text,
            evaluation=evaluation,
        )
        now = _iso_now()
        ...
        content_hash = _prefixed_hash(output["content"])
        ...
        return {
            ...
            "revision": {
                ...
                else "Regenerated tailored resume artifact.",
                "createdAt": now,
            },
            "uploadMetadata": None,
    ...
    def _enforce_truthfulness(
        ...
        *,
        ...
        if unsupported_claims:
            raise ResumeArtifactTruthfulnessError(
                "Resume generator reported unsupported claims"
            )
        ...
        for keyword in evaluation.missing_keywords or []:
            ...
            if normalized and normalized in content and normalized not in source:
                raise ResumeArtifactTruthfulnessError(
                    "Generated resume included a missing keyword unsupported by source"
                )
    ...
    def _validate_contract(self, artifact: dict[str, Any]) -> None:
        try:
            ...
            raise ResumeArtifactValidationError(
                f"Resume artifact contract validation failed: {exc.message}"
            ) from exc
...
def _resume_artifact_validator() -> Draft7Validator:
    schema_path = (
...
def _source_resume_metadata(source_version: ResumeSourceVersion) -> dict[str, Any]:
    return {
