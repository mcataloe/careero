...
from app.api.resume_artifacts import get_resume_artifact_service
...
class FakeResumeGenerator:
    def __init__(
        ...
        *,
        ...
        self.content = content or (
            "# Matthew Cataldi\n\n"
            "## Experience\n\n"
            "- Built Python services and PostgreSQL-backed platforms.\n"
        )
    ...
    def generate(self, **kwargs) -> dict[str, Any]:
        ...
        return {
            ...
            "output": {
                "title": "Example Co tailored resume",
                "content": self.content,
                "tailoring_notes": "Emphasize source-supported platform work.",
                "warnings": [],
                "limitations": [],
...
@pytest.fixture
def resume_artifact_api_client(
    seeded_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> Generator[TestClient, None, None]:
    monkeypatch.setenv("CAREERO_ENABLE_AI_RESUME_GENERATION", "false")
...
@pytest.fixture
def resume_artifact_api_client_with_fake_generator(
    seeded_session: Session,
    monkeypatch: pytest.MonkeyPatch,
) -> Generator[TestClient, None, None]:
    monkeypatch.setenv("CAREERO_ENABLE_AI_RESUME_GENERATION", "false")
    ...
    def override_service() -> ResumeArtifactService:
        return ResumeArtifactService(
            seeded_session,
    ...
    app.dependency_overrides[get_db] = override_get_db
    app.dependency_overrides[get_resume_artifact_service] = override_service
    with TestClient(app) as client:
        yield client
    app.dependency_overrides.clear()
    get_settings.cache_clear()
...
def create_role(db_session: Session):
    return RoleService(db_session).create_role(
        RoleCreate(
            ...
            remote_type="hybrid",
            raw_description=(
                "Build Python and PostgreSQL platforms for internal teams."
            ),
            normalized_description=(
                "Senior platform role using Python and PostgreSQL."
            ),
            status="found",
        )
    )
...
def generate_artifact(
    ...
    *,
    ...
    service = ResumeArtifactService(
        db_session,
        settings=Settings(_env_file=None),
        generator=generator or FakeResumeGenerator(),
    )
...
def test_service_creates_and_persists_valid_resume_artifact(
    seeded_session: Session,
) -> None:
    ...
    assert artifact["artifactType"] == "tailored_resume"
    assert artifact["formatMetadata"]["primaryFormat"] == "md"
    assert artifact["formatMetadata"]["availableFormats"] == ["md"]
    assert artifact["generationMetadata"]["generatedBy"] == "ai"
    assert artifact["generationMetadata"]["modelMetadata"]["model"] == "gpt-5-mini"
    assert artifact["generationMetadata"]["promptVersion"]
    assert artifact["generationMetadata"]["inputHash"].startswith("sha256:")
...
def test_api_generates_resume_artifact_with_expected_links(
    ...
    response = resume_artifact_api_client_with_fake_generator.post(
...
def test_missing_source_resume_fails_without_persisting_artifact(
    ...
    with pytest.raises(ResumeArtifactSourceNotFoundError):
        generate_artifact(seeded_session, role_id=role.id)

    assert seeded_session.query(GeneratedArtifact).count() == 0


def test_ai_unavailable_fails_without_persisting_artifact(
    seeded_session: Session,
) -> None:
    role = create_role(seeded_session)
    create_source(seeded_session)
    ...
    with pytest.raises(Exception) as exc_info:
        generate_artifact(
            seeded_session,
            role_id=role.id,
            generator=FakeResumeGenerator(unavailable=True),
        )
...
def test_validation_failure_rolls_back_and_logs_failure(
    ...
    with pytest.raises(ResumeArtifactValidationError):
        generate_artifact(
            seeded_session,
            role_id=role.id,
            generator=FakeResumeGenerator(unsupported_claims=["Invented AWS work"]),
        )
    ...
    actions = list(
        seeded_session.scalars(
            select(ActivityLog.action).where(
                ActivityLog.entity_type == "resume_artifact",
            )
        )
    )
    assert "resume_artifact.failed" in actions
...
def test_missing_keyword_not_supported_by_source_is_rejected(
    ...
    with pytest.raises(ResumeArtifactTruthfulnessError):
        generate_artifact(
            seeded_session,
            role_id=role.id,
            generator=FakeResumeGenerator(
                content=(
                    "# Matthew Cataldi\n\n"
                    "- Built Kubernetes, Python, and PostgreSQL platforms.\n"
                )
            ),
        )
...
def test_api_returns_503_when_ai_resume_generation_is_disabled(
    resume_artifact_api_client: TestClient,
    seeded_session: Session,
) -> None:
    role = create_role(seeded_session)
    create_source(seeded_session)
    ...
    response = resume_artifact_api_client.post(
        f"/api/roles/{role.id}/resume-artifacts",
...
def test_invalid_role_or_evaluation_returns_404(
    resume_artifact_api_client: TestClient,
    seeded_session: Session,
) -> None:
    role = create_role(seeded_session)
    ...
    missing_role_response = resume_artifact_api_client.post(
        f"/api/roles/{uuid4()}/resume-artifacts",
        json={"workspace_id": str(WORKSPACE_ID)},
    )
    missing_evaluation_response = resume_artifact_api_client.post(
        f"/api/roles/{role.id}/resume-artifacts",
        json={
            "workspace_id": str(WORKSPACE_ID),
