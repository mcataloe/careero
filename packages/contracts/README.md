...
- Provider-neutral COMPASS evaluation engine helpers.
...
Layer 3B adds a reusable provider-neutral COMPASS engine:
...
The engine accepts canonical `Workspace` and `Opportunity` objects plus optional resume/profile content. It calls an injected model provider, parses raw JSON, validates the result with `CompassEvaluationSchema`, and returns both the raw model output and the normalized validated evaluation. Invalid JSON, schema failures, and provider errors return a valid failed fallback evaluation instead of a completed evaluation.
