...
| Opportunity records and legacy Role compatibility identifiers | User | Medium | Retain while opportunity exists. | Export Opportunity-facing fields and compatibility identifiers where needed. | Delete/archive according to approved opportunity lifecycle. | Log create/update/archive and compatibility target IDs. | Yes, as source-grounded input. | Usage limits may count records; no employer-paid prioritization. |
