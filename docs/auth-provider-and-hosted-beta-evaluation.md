...
Layer 11A, Layer 11B, and local-first Layer 11.4 through 11.7 foundations.

It is evaluation only. It does not choose a final auth provider, add
dependencies, implement login/signup, create sessions/JWTs/OAuth/passkeys, or
claim hosted readiness.

## 1. Current Local-First Identity Reality

Careero currently uses a seeded local user, seeded local workspace, local
current-user context, and service-level ownership-boundary helpers. It also now
has local-first data export, lifecycle request records, AI usage event metering,
and entitlement boundary reporting.

This is not production authentication. It is not hosted tenant isolation. It
does not provide account recovery, login, signup, OAuth, passkeys, password
storage, support/admin access, or production multi-user identity.
...
| First-party email/password | Maximum control and no auth vendor dependency. | High. Requires password hashing, recovery, abuse controls, email verification, rate limits, monitoring, and security response. | Careero becomes directly responsible for credential security. | High. | Low. | Hosted mode only; local mode should not need credentials. | Weak unless security work is prioritized. | Possible but costly. | Security burden is easy to underestimate. |
...
| Single-user hosted app | Auth for one owner, hosted DB, secrets management, backup/restore, monitoring, export/delete policy. | Local current-user abstraction and local owner boundaries prepare some service seams. | Auth provider, account recovery, hosted deployment, backups, monitoring, incident process. | Medium. | Private career data leaves local device. | Medium. | Optional controlled experiment after explicit user decision. |
| Private hosted beta | Real auth, account recovery, tenant isolation tests, support process, monitoring, export/delete/retention policy, backups. | Local export, lifecycle requests, usage metering, entitlement boundaries, readiness reporting. | Production auth, tenant isolation certification, retention enforcement, deletion enforcement, backup/restore, monitoring, support, legal/privacy review. | Medium to high. | High unless access boundaries and logging are hardened. | Medium to high. | After local value and lifecycle expectations are stable. |
...
3. Decide whether the next hosted target is a single-user hosted experiment or
   private hosted beta.
4. Choose an auth provider only after account lifecycle expectations, recovery
   posture, and data ownership policy are stable.
...
This document adds no auth dependency, login/signup UI, session model, JWT,
OAuth flow, passkey flow, password storage, hosted tenant isolation claim,
billing integration, or hosted deployment infrastructure.
