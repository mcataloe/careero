...
The current layer status table and detailed layer notes live in [product strategy](00_product-strategy.md). The actionable build-order sequence, prompt order, readiness gates, and pull-forward rules live in [revised build order execution plan](07_revised-build-order-execution-plan.md). The navigable layer-by-layer entry point is [the layer index](../02_layers/00_layer-index.md).
