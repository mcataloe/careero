...
| Layer 7 | Opportunity Model Strategy | Layer 7B compatibility surface hardened / Layer 7C deferred | Opportunity-facing API and UX aliases are hardened for current local MVP use while persistence remains Role-backed. Destructive rename remains future. |
...
Layer 7 defines Opportunity as the central durable intelligence object. Layer 7B has hardened the Opportunity-facing API/UX compatibility surface while persistence remains Role-backed. The destructive Role-to-Opportunity persistence rename remains a separate future Layer 7C decision.
